...
bgdems <- st_read("C:/Users/a0uppa01/University of Louisville/Envirome Institute members_group - General/GIS/GreenHeart/ProcessedData/GHClusters_blckgrp.shp")
bgdems$FIPS <- as.numeric(bgdems$FIPS)
whichbg <- whichbg %>% select(StudyID, FIPS)
participants <- left_join(participants, whichbg, by=c("stid"="StudyID")) # FIPS.x
participants <- left_join(participants, bgdems, by=c("ZoneID"="ZoneID")) # FIPS.y
participants <- participants %>%
  rename(
    Per_Capita_Income = Per_Capita,
    Median_HH_income = Median_HH_,
    Percent_Less_than_Equal_to_HS_Diploma = Percent_Le,
    Percentage_Some_College = Percentage,
    Percentage_Bachelors_and_Above = Percenta_1,
...
pg_Cluster %>% tbl_summary(by = ZoneID,
                           ...
                           type = list(Male ~ "continuous", Female ~ "continuous",
                                       Pct_White ~ "continuous",
                                       Pct_Black ~ "continuous",
                                       Pct_Hispanic ~ "continuous",
                                       Per_Capita_Income ~ "continuous",
                                       Median_HH_income ~ "continuous",
                                       Percent_Less_than_Equal_to_HS_Diploma ~ "continuous",
                                       Percentage_Some_College ~ "continuous",
                                       Percentage_Bachelors_and_Above ~ "continuous",
                                       Own ~ "continuous", Rent ~ "continuous"),
...
file1 <- read.csv("C:/Users/a0uppa01/Documents/GIS/GHClusters_blockgroup.csv")
greenness <- read.csv("C:/Users/a0uppa01/Documents/GIS/HEALgreennessMetrics_RY_110920.csv")
greenness <- greenness %>% select(StudyID, FocalStatistics_OutRaster_Full7inNDVI042020200m)
GISdata <- read.csv("C:/Users/a0uppa01/Documents/GIS/HEAL_addGISData_AU_20201202.csv")
GISdata <- GISdata %>% select(NEAR_DIST_MajorRoads, LASTCNT_MajorRoadsTraffic, 
                              PopDen, StudyID)
